-- Scopes the report to invoices sold in the last 30 days, per instruction
-- after the full-year backfill surfaced far more (371 invoices, ~1.19cr)
-- than expected. Kept as a standing filter (not a one-time delete of older
-- cached rows) so it self-maintains going forward and stays reversible if
-- the window ever needs to change again.
