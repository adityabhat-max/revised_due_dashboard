-- Zenoti also fires Invoice.Payments.Deleted when a payment is reversed,
-- carrying the same transaction that a prior Invoice.Payments.Added (or an
-- invoice's embedded transactions) already recorded. Without this, a
-- reversed payment stays counted forever, overstating Collected and
-- understating Due. Excludes any (invoice_id, transaction_id) reported as
-- deleted from the payment pool, regardless of which source recorded it.
...
open_invoices as (
  select
    zoi.invoice_id as invoice_id,
    coalesce(zoi.payload->>'invoice_number_prefix','') || coalesce(zoi.payload->>'invoice_number','') as invoice_no,
    (zoi.payload->>'invoice_date')::timestamptz as sale_date,
    trim(coalesce(zoi.payload->>'guest.first_name','') || ' ' || coalesce(zoi.payload->>'guest.last_name','')) as guest_name,
    zoi.payload->>'guest.code' as guest_code,
    zoi.payload->>'center_id' as center_id,
    (zoi.payload->>'total_price.sum_total')::numeric as invoice_sales_inc_tax,
    coalesce(zoi.payload->'invoice_items', '[]'::jsonb) as invoice_items,
    coalesce(zoi.payload->'additional_fields', '[]'::jsonb) as additional_fields,
    zoi.transactions as api_transactions
  from public.zenoti_open_invoices zoi
),
payments_added as (
  select
...
deleted_transactions as (
  ...
  where payload->>'event_type' = 'Invoice.Payments.Deleted'
),
all_payments_raw as (
  ...
  union all
  select invoice_id, txn->>'transaction_id' as transaction_id, txn->>'payment_date' as payment_date_raw, (txn->>'amount_paid')::numeric as amount_paid
  from open_invoices, jsonb_array_elements(api_transactions) txn
),
all_payments as (
  select distinct on (apr.invoice_id, apr.transaction_id)
    apr.invoice_id, apr.transaction_id, apr.payment_date_raw, apr.amount_paid
  from all_payments_raw apr
  left join deleted_transactions dt
    on dt.invoice_id = apr.invoice_id and dt.transaction_id = apr.transaction_id
  where dt.transaction_id is null
  order by apr.invoice_id, apr.transaction_id, apr.payment_date_raw
),
ranked_payments as (
  select
    invoice_id,
...
where (ir.invoice_sales_inc_tax - coalesce(ps.collected, 0)) > 0
  and ir.sale_date >= now() - interval '30 days'
