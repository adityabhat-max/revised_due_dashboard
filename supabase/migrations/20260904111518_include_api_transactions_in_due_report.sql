-- Feeds each open invoice's full API-fetched transaction history (now
-- captured via expand=Transactions, see zenoti_open_invoices.transactions)
-- into the same payment pool the view already dedupes by transaction_id.
-- Without this, a backfilled invoice (no webhook-observed payments at all)
-- would show Collected: 0 and overstate Due.
